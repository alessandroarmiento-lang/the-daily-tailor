...
6. **Action emails** — IMAP iCloud+Gmail (Mac-off) or Mail.app (Mac awake); rolling newest actionable only (default 4).
...
| `ACTION_EMAIL_SOURCE` / `ACTION_EMAIL_MAX_ITEMS` / `ACTION_EMAIL_LOOKBACK_DAYS` | `auto` / `4` / `14` | `auto`\|`imap`\|`applemail`\|`mock`; rolling newest-N actionable in lookback |
